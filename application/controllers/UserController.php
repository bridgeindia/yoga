...
class UserController extends Zend_Controller_Action
{
  ...
  public function loginformAction()
	...
	/**
	 * function that displays the warning to enter the settings details , in a popup
	 * @params workout id
	 * @author lekha
	 * @date 4/25/2012
    ...
    public function checksettingsAction()
	{
	 ...
	 /**
	 * function that displays the warning to enter the settings details with layout
	 * @params workout id
	 * @author lekha
	 * @date 4/25/2012
	 
	 */ 
    public function checksettingspageAction()
	{
		$this->view->loginStatus  = $this->isLoggedIn();
  ...
  public function playvideoAction()
  {
	...
	$totalExerciseCount   = count($exerciseArray);
	  	...
	  	foreach($exerciseArray as $exercise)
	  	{
			...
			//insert the recovery here
					...
					if(file_exists('./public/js_old/old_files/web/'.$videoexplode.'-2.mp4'))
					{
					    $repetitionArray[] =  1;
						$repetitionArray[] =  1;
						$repetitionArray[] =  1;
					...
					//{   
					    if(($workoutInfo['work_recovery_interval'] == 1) && ($m!=$totalExerciseCount))
							...
							if(($workoutInfo['work_recovery_interval'] == 2) && ($m!=$totalExerciseCount))
							{
								if(($m%2) == 0)
								{
									$repetitionArray[] =  1;
							...
							if(($workoutInfo['work_recovery_interval'] == 3)&& ($m!=$totalExerciseCount))
							{
								if(($m%3) == 0)
								{
									$repetitionArray[] =  1;
	...
	$this->view->fullpurchase       =  $userDetails['workout_purchase'];
	$this->view->workoutStatus    = $workoutInfo['status'];
...
function sendNotification($auth,$message,$devicetoken,$sender="",$level="",$plan)
        {
