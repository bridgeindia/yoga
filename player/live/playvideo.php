...
playVideos('<?php echo $_POST["siteurl"] ?>','<?php echo $_POST["videoStr"] ?>','<?php echo $_POST["repStr"] ?>',<?php echo $_POST["user_id"] ?>,<?php echo $_POST["workout_id"] ?>,<?php echo $_POST["upgrade"] ?>);
   ...
   function playVideos(siteurl,movieStr,repStr,userid,workoutid,upgrade)
   {
   	var siteurl  =  siteurl + '/public/js_old/old_files/web/';
...
if(reps == repetition[count])
	{
		...
		req.open("GET","ajax_workoutUpdate.php?userid="+userid+"&workoutid="+workoutid+"&timeDone="+currentDuration,false);
	    req.send("userid="+userid+"&workoutid="+workoutid+"&timeDone="+currentDuration);
	     ...
	     {   
			  ...
			  if(upgrade==1)
			  window.location.href = "http://fitness4.me/user/paymentoptions/";
			  else
			  jwplayer().load(siteurl+files[count]);
