...
if(($_GET["login"] !="") && ($_GET["token"]==$authToken) && ($_GET["email"] !="")&& ($_GET["password"] !=""))
  ...
  if(($_GET["checkusername"] !="") && ($_GET["token"]==$authToken) && ($_GET["email"] !=""))
  {
    $newObject->checkusername($_GET["email"]);
  }
  
  if(($_GET["iphoneRegister"] !="") && ($_GET["token"]==$authToken) && ($_GET["user_id"] !="") && ($_GET["user_id"] !=0))
  {
  ...
  if(($_GET["yogaallvideos"] !="") && ($_GET["token"]==$authToken)&& ($_GET["userid"] !=""))
  {
    if($_GET["lang"] == "")
	{
		$lang  =  1;
    ...
    $newObject->listallvideos($_GET["userid"],$lang);
  }
